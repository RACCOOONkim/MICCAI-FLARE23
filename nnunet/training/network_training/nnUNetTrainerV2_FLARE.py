...
class nnUNetTrainerV2_FLARE_Fine(nnUNetTrainerV2):
...
class nnUNetTrainerV2_FLARE_Swift(nnUNetTrainerV2):
    def initialize_network(self):
        ...
        print("conv: ", self.net_conv_kernel_sizes)
        print("pool: ", self.net_num_pool_op_kernel_sizes)
        ...
        net_nonlin = nn.LeakyReLU
        net_nonlin_kwargs = {'negative_slope': 1e-2, 'inplace': True}
        print("net pool size :", self.net_num_pool_op_kernel_sizes)
        self.network = Generic_UNet(self.num_input_channels, self.base_num_features, self.num_classes,
                                    3,
                                    self.conv_per_stage, 2, conv_op, norm_op, norm_op_kwargs, dropout_op, dropout_op_kwargs,
                                    net_nonlin, net_nonlin_kwargs, True, False, lambda x: x, InitWeights_He(1e-2),
