...
class GenericPreprocessor(object):
    ...
    def preprocess_test_case(self, data_files, target_spacing, seg_file=None, force_separate_z=True): # True
    ...
    def run(self, target_spacings, input_folder_with_cropped_npz, output_folder, data_identifier,
        ...
        maybe_mkdir_p(os.path.join(output_folder, data_identifier + "_stage1"))
        list_of_preprocessed_npz_files = subfiles(output_folder+'/'+data_identifier+"_stage1", True, None, ".npz", True)
        prepro_case_identifier = []
        new_list_of_cropped_npz_files = []
        for j, case in enumerate(list_of_preprocessed_npz_files):
                print("case: ", case)
                case_identifier = get_case_identifier_from_npz(case)
                prepro_case_identifier.append(case_identifier)
        for j, case in enumerate(list_of_cropped_npz_files):
            case_identifier = get_case_identifier_from_npz(case)
            if case_identifier not in prepro_case_identifier:
                new_list_of_cropped_npz_files.append(list_of_cropped_npz_files[j])
                print("not done : ", case_identifier)
            else:
                print("already done : ", case_identifier)
        print("still left: ", len(new_list_of_cropped_npz_files))
        ...
        for i in range(num_stages):
            ...
            for j, case in enumerate(new_list_of_cropped_npz_files):
                case_identifier = get_case_identifier_from_npz(case)
...
class PreprocessorFor2D(GenericPreprocessor):
    ...
    def run(self, target_spacings, input_folder_with_cropped_npz, output_folder, data_identifier,
        ...
        print("output_folder:", output_folder)
        list_of_cropped_npz_files = subfiles(input_folder_with_cropped_npz, True, None, ".npz", True)
        list_of_preprocessed_npz_files = subfiles(output_folder+'/nnUNetData_plans_v2.1_2D_stage0', True, None, ".npz", True)
        prepro_case_identifier = []
        for j, case in enumerate(list_of_preprocessed_npz_files):
                case_identifier = get_case_identifier_from_npz(case)
                prepro_case_identifier.append(case_identifier)
        ...
        for i in range(num_stages):
            ...
            for j, case in enumerate(list_of_cropped_npz_files):
                case_identifier = get_case_identifier_from_npz(case)
                if case_identifier in prepro_case_identifier:
                    print("already done : ", case_identifier)
                    continue
                args = spacing, case_identifier, output_folder_stage, input_folder_with_cropped_npz, force_separate_z, all_classes
                all_args.append(args)
