...
class SegmentationNetwork(NeuralNetwork):
    ...
    def _internal_predict_3D_3Dconv_tiled(self, x: np.ndarray, step_size: float, do_mirroring: bool, mirror_axes: tuple,
        ...
        for z in steps[0]:
            ...
            # xy 평면에는 총 9개의 윈도우가 있으며, 중앙의 1개 윈도우에 대한 추론을 진행
            lb_z = z
            ub_z = z + patch_size[0]
            ...
            # 요청에 따라 lb_x 값을 수정 
            # steps[2][1] 값이 없을 경우 steps[2][0] 값을 사용
            if len(steps[2])==1:
                steps[2].append(0)
            lb_x = steps[2][1] 
            ...
            # 윈도우가 완전히 비어 있지 않거나 반 이상 비어 있지 않은 경우
            if not (is_empty or is_half_empty):
                ...
                for y in steps[1]:
                    ...
                    for x in steps[2]:
                        ...
                        # 중앙의 윈도우는 이미 처리했으므로 다시 처리하지 않습니다.
                        if y == steps[1][1] and x == steps[2][1]:
                            continue
